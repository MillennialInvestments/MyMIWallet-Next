...
-- Column adjustments
SET @col_exists := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'bf_roles' AND column_name = 'slug');
SET @sql := IF(@col_exists = 0, 'ALTER TABLE `bf_roles` ADD COLUMN `slug` TEXT NULL', 'SELECT ''column slug already exists''');
