...
- Error: `Pull request already exists for this branch`
  - Cause: GitHub permits one open PR per source branch into the same base branch.
  - Action: push follow-up commits to the current branch to update that existing PR, or create a new branch for a new PR thread.
  - AIOps guidance: treat this response as non-fatal and switch to "update existing PR" mode in automation.
