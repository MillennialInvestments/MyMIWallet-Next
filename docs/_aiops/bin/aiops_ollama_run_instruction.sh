...
{
  echo
  echo "## Forced Local AIOps/Ollama Run Context"
  echo
  echo "- Run ID: ${RUN_ID}"
  ...
  echo "- Model: ${MODEL}"
  echo "- Required behavior: use local Ollama only."
  echo "- Required output: produce actual code changes or a specific failure reason."
  echo "- Avoid duplicate-ingest blocking by using this fresh run copy."
  echo "- Do not commit writable/logs."
  echo "- Do not create broad docs churn."
} >> "$FRESH_INSTRUCTION"
...
{
  echo "===== GLOBAL AIOPS / LOCAL OLLAMA RUN ====="
  date -u
...
echo
echo "===== SUMMARY ====="
grep -Ei "status|ok|degraded|down|503|duplicate|near-duplicate|queued|runnable|No runnable|Blocked recursive|worker|patch|failed|invalid|error|exception|created|modified|updated|FINAL STATUS|FINAL DIFF STAT|No syntax errors" "$REPORT" | head -n 360 || true
