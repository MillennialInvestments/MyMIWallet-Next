...
let WorkflowRepository = class WorkflowRepository extends typeorm_1.Repository {
    ...
    getManyQuery(workflowIds, options = {}) {
        ...
        this.applyTriggerNodeTypeFilter(qb, options.filter?.triggerNodeType);
    ...
    applyTriggerNodeTypeFilter(qb, triggerNodeType) {
        if (triggerNodeType) {
            const dbType = this.globalConfig.database.type;
            ...
            if (['postgresdb'].includes(dbType)) {
                qb.andWhere('COALESCE("activeVersion"."nodes"::text, "workflow"."nodes"::text) LIKE :triggerNodeType', { triggerNodeType: `%${triggerNodeType}%` });
            }
            else {
