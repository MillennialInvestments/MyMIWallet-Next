...
async function executeBatch(ctx, batch, startIndex, model, fallbackModel, memory, response) {
  ...
  (0, import_n8n_workflow.assertParamIsNumber)("options.maxIterations", maxIterations, ctx.getNode());
  const batchPromises = batch.map(async (_item, batchItemIndex) => {
    const itemIndex = startIndex + batchItemIndex;
    (0, import_checkMaxIterations.checkMaxIterations)(response, maxIterations, ctx.getNode());
