...
let ChatHubWorkflowService = class ChatHubWorkflowService {
    ...
    getSystemMessageMetadata(timeZone) {
        const now = luxon_1.DateTime.now().setZone(timeZone).toISO({
            includeOffset: true,
        });
...
${this.getSystemMessageMetadata(timeZone)}`;
    ...
    async buildRestoreMemoryNode(history) {
    ...
    async buildMessageValuesWithAttachments(history) {
        ...
        for (const message of messages) {
            ...
            for (const attachment of attachments) {
                const block = await this.buildContentBlockForAttachment(attachment, currentTotalSize, maxTotalPayloadSize);
                blocks.push(block);
    ...
    async buildContentBlockForAttachment(attachment, currentTotalSize, maxTotalPayloadSize) {
