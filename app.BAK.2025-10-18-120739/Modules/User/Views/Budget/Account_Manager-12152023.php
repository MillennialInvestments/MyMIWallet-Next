...
if (json_last_error() != JSON_ERROR_NONE) {
    ...
    echo "Error in JSON format: " . json_last_error_msg();
...
if ($formMode === 'Add') {
    if ($is_debt === 1) {
        ...
        if ($existingRecordCheck->num_rows() > 0) {
            ...
            // Add or modify the fields as per your requirement
            $updateData = array(
                'status'                            => $status,
                'date'                              => $date,
                'time'                              => $time,
            ...
            if (!$this->db->update('bf_users_debt_accounts', $updateData)) {
                ...
                if ($this->db->insert('bf_users_budgeting', $accountData)) {
                    ...
                    log_message('error', 'Database bf_users_budgeting failed: ' . $this->db->error_message());
                }
            }
        } else {
            $debtData                                   = array(
            ...
            if (!$this->db->insert('bf_users_debt_accounts', $debtData)) {
                log_message('error', 'Database insert failed: ' . $this->db->error_message());
            } else {
                if ($this->db->insert('bf_users_budgeting', $accountData)) {
                    if ($accountData['recurring_account'] === 'Yes') {
                        $accountID                              = $this->db->insert_id(); 
