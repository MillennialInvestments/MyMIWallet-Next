...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const fetchAndStore = async (url, key) => {
        try {
            ...
            if (json.status === 'success') {
                ...
                console.error(`❌ Failed to load ${key}`, json);
            }
        } catch (err) {
            console.error(`⚠️ Error fetching ${key}:`, err);
        }
    };

    // Load all datasets via API
    const fetchAllBudgetData = async () => {
        await Promise.all([
            fetchAndStore('/index.php/Budget/getUserBudgetRecords', 'budgetData'),
            fetchAndStore('/index.php/Budget/getUserCreditBalances', 'creditData'),
            fetchAndStore('/index.php/Budget/getUserAvailableBalances', 'availableData'),
            fetchAndStore('/index.php/Budget/getUserRepaymentSummary', 'repaymentSummary')
        ]);
    ...
    const calculateSurplus = (expenses, incomes) => {
        return incomes.map((income, i) => (income || 0) - (expenses[i] || 0));
    };

    const calculateCumulativeSurplus = (surpluses) => {
        const cumulative = [];
        surpluses.reduce((acc, surplus) => {
            const cumulativeValue = acc + surplus;
    ...
    const calculateSurplusColors = (surpluses) => {
        return surpluses.map(value => (value >= 0 ? '#1ee0ac' : '#e85347'));
    };
    ...
    const updateChartData = () => {
        ...
        const surpluses = calculateSurplus(expenses, incomes);
        const cumulativeSurplus = calculateCumulativeSurplus(surpluses);
        const surplusColors = calculateSurplusColors(cumulativeSurplus);

        console.log('📊 Chart Range:', { startDate, endDate });
        chart.data.labels = getMonthLabels(startDate, endDate, true);
        chart.data.datasets[0].data = surpluses;
        chart.data.datasets[1].data = incomes;
        chart.data.datasets[2].data = expenses;
        chart.data.datasets[3].data = cumulativeSurplus;
        chart.data.datasets[3].borderColor = surplusColors;
        chart.data.datasets[3].pointBackgroundColor = surplusColors;
        chart.update();
    };
