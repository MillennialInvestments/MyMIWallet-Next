    ...
    public function run(array $params)
    {
        ...
        if ($breaker->isOpen()) {
            return $this->emitPayload(
                ['error' => 'Circuit open. Ollama temporarily disabled.'],
                $flags,
                self::EXIT_CRITICAL
            );
        }
        ...
        try {
            ...
            $res = $this->client()->generate(
                $payload,
                $this->resolveBaseUrl($flags),
                $this->resolveTimeout($flags)
            );
            ...
            log_message(
                'error',
                'ollama:generate failed {run_uuid}: {error}',
                ['run_uuid' => $runUuid, 'error' => $e->getMessage()]
            );
