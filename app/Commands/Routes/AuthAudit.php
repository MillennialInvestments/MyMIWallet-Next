...
class AuthAudit extends SafeBaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        foreach ($this->publicRoutes as $route) {
            ...
            if ($result['status'] === 'fail') {
                $failures++;
                CLI::write(sprintf('[FAIL] %s %s → %s', $route['method'], $route['path'], $result['message']), 'red');
                continue;
            }

            if ($result['status'] === 'warn') {
                $warnings++;
                CLI::write(sprintf('[WARN] %s %s → %s', $route['method'], $route['path'], $result['message']), 'yellow');
                continue;
            }

            CLI::write(sprintf('[PASS] %s %s → %s', $route['method'], $route['path'], $label), 'green');
        }
