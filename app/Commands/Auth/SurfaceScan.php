...
class SurfaceScan extends SafeBaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        CLI::write('AuthController: ' . ($report['auth_controller_namespace'] ?? 'n/a'));
        ...
        CLI::newLine();
        CLI::write('Login/Register routes: ' . count($report['login_register_routes'] ?? []));
        foreach (($report['login_register_routes'] ?? []) as $route) {
            CLI::write('- ' . ($route['method'] ?? '?') . ' ' . ($route['uri'] ?? '?'));
        }

        CLI::write('Dashboard redirect routes: ' . count($report['dashboard_routes'] ?? []));
        foreach (($report['dashboard_routes'] ?? []) as $route) {
            CLI::write('- ' . ($route['method'] ?? '?') . ' ' . ($route['uri'] ?? '?'));
        }

        CLI::write('Filters touching auth/dashboard: ' . count($report['filters_touching_auth_dashboard'] ?? []));
