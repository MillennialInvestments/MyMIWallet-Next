...
||||||| parent of 0ad077f44 (01/23/2026 - DbApplyDocs.php - TBJ)
=======
<?php
...
class DbApplyDocs extends BaseCommand
{
    ...
    protected $name  = 'db:apply-docs';
    protected $description = 'Apply SQL extracted from docs/mysql/*.md';

    public function run(array $params)
    {
        $root = rtrim(ROOTPATH, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;

        $script = $root . 'scripts/extract_sql_from_md.sh';
        if (! is_file($script)) {
            CLI::error("Extractor script not found: {$script}");
            return;
        }

        CLI::write("Extracting SQL from docs/mysql…", 'yellow');
        passthru('bash ' . escapeshellarg($script));

        $sqlFile = $root . 'writable/db_inventory/compiled_adjustments.sql';
        if (! is_file($sqlFile)) {
            CLI::error("Compiled SQL not found: {$sqlFile}");
            return;
        }
        ...
        CLI::write("Applying DB adjustments…", 'yellow');
        ...
        file_put_contents($logFile, implode(PHP_EOL, $output));
        

        $logDir = WRITEPATH . 'db_inventory/';
        if (! is_dir($logDir)) {
            mkdir($logDir, 0755, true);
        }

        if ($return === 0) {
            $logFile = $logDir . 'apply_' . date('Y-m-d_His') . '.log';
            if ($return !== 0) {
                CLI::error("Failed to apply DB adjustments. See log: {$logFile}");
                return;
            }
            CLI::write("✔ Database schema verified and aligned", 'green');
            CLI::write("Log written to: {$logFile}", 'blue');
        } else {
            CLI::error("Schema apply encountered errors. See log: {$logFile}");
        }

    }
}
>>>>>>> 0ad077f44 (01/23/2026 - DbApplyDocs.php - TBJ)
