...
<div class="modal fade" id="transactionModal" tabindex="-1" aria-labelledby="trackDepositsModal" aria-hidden="true">
    <div class="modal-dialog modal-xl" id="transModalDialog">
        <div class="modal-content" id="loading-content">
            <?php echo view('UserModule\Views\Dashboard\index\modal-loading-page'); ?>
        </div>
...
function initializePayPalButton() {
    paypal.Buttons({
        createOrder: function(data, actions) {
            // Extract promo code from input or other source
            const promoCode = $('#promo_code_input').val() || '';
            const transCostElement = document.getElementById("transCost");
            const transCost = parseFloat(transCostElement ? transCostElement.innerText.replace(/,/g, '') : '0') || 0;
            ...
            return actions.order.create({
                purchase_units: [{
                    ...
                    custom_id: promoCode // Pass promo code to PayPal
                }]
            });
        },
        onApprove: function(data, actions) {
            return actions.order.capture().then(function(details) {
                ...
                formData.append('promo_code', details.purchase_units[0].custom_id); // Pass promo code back to server
