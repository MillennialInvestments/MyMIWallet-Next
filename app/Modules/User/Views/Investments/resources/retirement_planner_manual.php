                ...
                <div class="card-inner">
                    <div class="card-title-group mb-3">
                        <div class="card-title">
                            <h6 class="title">Retirement Planner Manual</h6>
                        </div>
                    </div>
                    <form method="post">
                    ...
                    <div class="nk-order-ovwg pb-5">
                        <h6>Basic Retirement Projection</h6>
                        <ul>
                            <li>Years to Retirement: <?= $basicProjection['years_to_retirement'] ?></li>
                            <li>Estimated Savings at Retirement: $<?= number_format($basicProjection['estimated_savings'], 2) ?></li>
                        </ul>
                    </div>
                    <div class="nk-order-ovwg pb-5">
                        <h6>Retirement Recommendations</h6>
                        <ul>
                    ...
                    <div class="nk-order-ovwg pb-5">
                        <h6>Retirement Savings Projection</h6>
                        <canvas id="retirementProjectionChart"></canvas>
                        <script <?= $nonce['script'] ?? '' ?>>
                            document.addEventListener('DOMContentLoaded', function () {
                                const ctx = document.getElementById('retirementProjectionChart').getContext('2d');
                                const chart = new Chart(ctx, {
                                    type: 'line',
                                    data: {
                                        labels: <?= json_encode($visualizations['age']) ?>,
                                        datasets: [
                                            {
                                                label: 'Estimated Savings',
                                                data: <?= json_encode($visualizations['savings']) ?>,
                                                borderColor: '#1ee0ac',
                                                borderWidth: 2,
                                                fill: false
                                            },
                                            {
                                                label: 'Savings Goal',
                                                data: <?= json_encode($visualizations['goal']) ?>,
                                                borderColor: '#e85347',
                                                borderWidth: 2,
                                                fill: false,
                                                borderDash: [5, 5]
