...
<script <?= $nonce['script'] ?? '' ?>>
    ...
    document.querySelector('#trade_type').addEventListener('change', function() {
        ...
        // Initialize the select picker with any necessary options here
        // $('.selectpicker').selectpicker();

        $('#trade_type').on('change', async function() {
            const selectedType = this.value;
            const specificFields = $('#specific_fields');
            specificFields.empty(); // Clear previous fields before adding new

            // Assuming fields array is prepared elsewhere or within this block
            let fieldsHtml = ''; // Prepare your dynamic fields HTML string here based on selectedType

            // Simulate adding the select picker for Symbol dynamically
            fieldsHtml += addSelectPicker('Symbol', 'symbol', 'Select or Search Symbol', phpVar.symbolValue, []);
            specificFields.html(fieldsHtml);

            // Re-initialize the select picker for the dynamically added select
            $('.selectpicker').selectpicker('refresh');

            // Populate symbols based on trade type
            await populateSymbolPicker(selectedType);
        });

        // Ensure dynamic addition of options works correctly
        $('.bs-searchbox input').on('keyup', function(e) {
            let inputVal = $(this).val();
            let selectPicker = $(this).closest('.bootstrap-select').find('select');

            // Option not found, add it dynamically
            if (selectPicker.find(`option[value="${inputVal}"]`).length === 0 && inputVal !== '') {
                // Remove the previous dynamically-added option
                selectPicker.find('option.dynamic-option').remove();
                // Add the new option
                selectPicker.prepend(`<option class="dynamic-option" value="${inputVal}">${inputVal}</option>`).selectpicker('refresh');
                // Automatically select the newly added option
                selectPicker.selectpicker('val', inputVal);
            }
        });
