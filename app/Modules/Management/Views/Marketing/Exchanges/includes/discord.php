                            ...
                            <div class="card card-bordered pb-4 pricing px-2">
                                <div class="pricing-head">
                                    <div class="pricing-title">
                                        ...
                                        <p class="sub-text">List of top communities on Discord.</p>
                                    </div>
                                </div>
                                    ...
                                    <table id="discord-table" class="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                            <?php foreach ($tokens as $token): ?>
                                                <tr>
                                                    <td><a href="<?= $token['website'] ?>" target="_blank"><?= $token['coin_name'] ?></a></td>
                                                    <td><?php echo '$' . number_format((float)$token['market_cap'], 0); ?></td>
                                                    <td>
                                                        <a href="<?= $token['discord'] ?>" target="_blank">Discord</a>
                                                        <a href="<?php echo site_url('/Management/Marketing/Top-Communities/Active/' . $token['id']); ?>" target="_blank"><em class="icon myfs-md ni ni-check text-success text-bold pl-2"></em></a>
                                                        <a href="<?php echo site_url('/Management/Marketing/Top-Communities/Inactive' . $token['id']); ?>" target="_blank"><strong><em class="icon myfs-md ni ni-cross text-danger text-bold pl-2"></em></strong></a>
                                                    </td>
                                                    <td>
                                                        <div class="dropdown">
...
<script <?= $nonce['script'] ?? '' ?>>
    $(document).ready(function() {
        initDataTableSafe($('#discord-table'),{
            "order": [[ 2, "desc" ]],     
