...
class ServicesAdminController extends BaseAdminController
{
    protected $auth;
...
//         $this->MyMIWallets                          = new MyMIWallets(); // replaced by BaseController getter 
    ...
    public function index()
    {
        ...
        $this->commonData();
        return $this->renderTheme('App\Modules\Management\Views\Services\index', $this->data);
    }

    public function campaigns()
    {
        $this->data['pageTitle'] = 'Management Marketing Campaigns | MyMI Wallet';
        $this->commonData();
        return $this->renderTheme('App\Modules\Management\Views\Services\campaigns', $this->data);
    }

    public function tasks()
    {
        $this->data['pageTitle'] = 'Management Marketing Tasks | MyMI Wallet';
        $this->commonData();
        return $this->renderTheme('App\Modules\Management\Views\Services\tasks', $this->data);
    }

    public function blogSubscribe()
    {
        $this->data['pageTitle'] = 'Management Marketing Blog Subscribe | MyMI Wallet';
        $this->commonData();
        return $this->renderTheme('App\Modules\Management\Views\Services\blog_subscribe', $this->data);
    }

    public function seoCheck()
    {
        $uriString = $this->request->getUri()->getPath();
        $query = $this->db->getWhere('bf_marketing_page_seo', ['url' => $uriString]);
        if ($query->getNumRows() == 0) {
            $data = [
                'url' => $uriString,
                'title' => str_replace(['_', '/'], [' ', ' - '], $uriString),
                'description' => '',
                'image' => '',
            ];
            $this->db->table('bf_marketing_page_seo')->insert($data);
            ...
            $this->assignMarketingTask($taskInfo);
            $html = file_get_contents(site_url($uriString));
            $dom = new \DOMDocument;
            libxml_use_internal_errors(true);
            $dom->loadHTML($html);
            libxml_clear_errors();
            $xpath = new \DOMXPath($dom);
            $descriptionElement = $xpath->query("//*[@id='page_seo_description']");
            $imageElement = $xpath->query("//*[@id='page_seo_image']");
            if ($descriptionElement->length > 0) {
                $data['description'] = $descriptionElement->item(0)->nodeValue;
            }
            if ($imageElement->length > 0) {
                $data['image'] = $imageElement->item(0)->getAttribute('src');
            }
            $this->db->table('bf_marketing_page_seo')->update($data, ['id' => $insertId]);
        }
    }

    public function assignMarketingTask($taskInfo)
    {
        $data = [
            'status' => 'Pending',
            'group' => 'Marketing',
            ...
            'description' => 'Complete SEO for ' . $taskInfo['url'],
            'url' => site_url('/Management/Marketing/Page-SEO/' . $taskInfo['seo_id']),
        ];
        $this->db->table('bf_management_tasks')->insert($data);
    }
