...
class MarketingAdminController extends BaseAdminController
{
    ...
    public function cronGenerateFinalContentFromTemp()
    {
        try {
            log_message('debug', 'Starting cronGenerateFinalContentFromTemp');
    
            $this->marketingService->generateFinalContentFromTemp();
            log_message('debug', 'Finished cronGenerateFinalContentFromTemp');
            return $this->respond(['status' => 'success']);
        } catch (\Throwable $e) {
            log_message('error', 'cronGenerateFinalContentFromTemp Error: ' . $e->getMessage());
            return $this->failServerError($e->getMessage());
        }
    }
