...
class BudgetsController extends UserController
{
    protected $auth;
...
//         $this->MyMIWallets                          = new MyMIWallets(); // replaced by BaseController getter 
    ...
    public function commonData(): array {  
        ...
        $this->data['promotionalBanners'] = $this->getDashboardService()->dashboardInfo($this->cuID)['promotionalBanners'];
    ...
    {
        $this->data['pageTitle'] = 'Assets | Management | MyMI Wallet';
        $this->commonData();
        return $this->renderTheme('App\Modules\Management\Views\Assets\index', $this->data);
    }

    public function applications()
    {
        $this->data['pageTitle'] = 'Applications | Management | MyMI Wallet';
        $this->commonData();
        return $this->renderTheme('App\Modules\Management\Views\Assets\applications', $this->data);
    }

    public function applicationDetails()
    {
        $this->data['pageTitle'] = 'Application Details | Management | MyMI Wallet';
        $this->commonData();
        return $this->renderTheme('App\Modules\Management\Views\Assets\application_details', $this->data);
    }
    ...
    public function approval($appID)
    {
        ...
        if ($assetStatus === 'Approved') {
            if ($this->getMyMIAnalytics()->migrateAssetRequestInfo($appID)) {
                $assetAppData = $this->analyticalModel->getApprovedAssetById($appID);
                $this->session->setFlashdata('message', 'Asset Application has been Approved!');
                return $this->renderTheme('App\Modules\Management\Views\Assets\approval', $assetAppData);
            }
        } elseif ($assetStatus === 'Denied') {
            if ($this->analyticalModel->updatePendingAssetStatus($appID, $assetStatus)) {
                $assetAppData = $this->analyticalModel->getApprovedAssetById($appID);
                $this->session->setFlashdata('message', 'Asset Application has been Denied!');
                return $this->renderTheme('App\Modules\Management\Views\Assets\approval', $assetAppData);
            }
        }
    }
    ...
    {
        $this->data['pageTitle'] = 'Create Asset | Management | MyMI Wallet';
        $this->commonData();
        return $this->renderTheme('App\Modules\Management\Views\Assets\create', $this->data);
    }

    public function distribute()
    {
        $this->data['pageTitle'] = 'Distribute Assets | Management | MyMI Wallet';
        $this->commonData();
        return $this->renderTheme('App\Modules\Management\Views\Assets\distribute', $this->data);
    }
