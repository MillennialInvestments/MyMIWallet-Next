...
class WebDesignAdminController extends BaseAdminController
{
    use ResponseTrait;
    use RequestTrait;
...
//         $this->MyMISecurity = new MyMISecurity(); // replaced by BaseController getter
        ...
        if (!$this->cuID) {
            log_message('warning', 'WebDesignController init aborted: no user id available.');
            return;
        }

        log_message('debug', 'WebDesignController initialized. Memory: ' . memory_get_usage(true));
