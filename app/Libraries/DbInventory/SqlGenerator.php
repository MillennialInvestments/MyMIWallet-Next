...
class SqlGenerator
{
    ...
    public function writeAdjustments(array $inventory, ?array $dbReport): void
    {
        ...
        $sql = $this->buildAdjustmentSql($inventory, $dbReport);
        file_put_contents($logPath, $sql);

        $missingTables = $this->buildMissingTablesSql($inventory, $dbReport);
        if ($missingTables !== '') {
            file_put_contents($docsPath . '/missing_tables.sql.md', $missingTables);
        }

        $missingColumns = $this->buildMissingColumnsSql($inventory, $dbReport);
        if ($missingColumns !== '') {
            file_put_contents($docsPath . '/missing_columns.sql.md', $missingColumns);
        }
    }
    ...
    private function buildTableDoc(string $table, array $schema, array $sources): string
    {
        ...
        if (! empty($sources)) {
            $sourceParts = [];
            foreach (['migrations', 'code'] as $type) {
                if (! empty($sources[$type])) {
                    $sourceParts[] = $type . ': ' . implode(', ', $sources[$type]);
                }
            }
            if ($sourceParts !== []) {
                $lines[] = '-- Sources: ' . implode(' | ', $sourceParts);
            }
        }

        if (! empty($schema['meta']['suspected_columns'])) {
            $lines[] = '-- Suspected columns: ' . implode(', ', $schema['meta']['suspected_columns']);
        }

        $lines[] = $this->buildCreateTableSql($table, $schema);
        $lines[] = '';
        $lines[] = '-- Column adjustments';
