...
if (data[1][0]) {
	...
	const tableGenerator = (target, data) => {
		...
		for (var i = 0; i < data.length; i++) {
			...
			columns.forEach((name) => {
				...
				column.innerHTML = "<a onClick=\"addOrder\">" + content[name] + "</a>";
	...
	let startSource = () => {
		evtSource = new EventSource(
			...
			//~  Localhost
			// `http://localhost/MillennialInvest/Site-v7/v1.4/public/index.php/Exchange/Order_Event_Manager/${url + id}`, { withCredentials: true }
			/*  Remote Localhost */
			`http://10.0.0.60/MillennialInvest/Site-v7/v1.4/public/index.php/Exchange/Order_Event_Manager/${url + id}`, { withCredentials: true }
			// `http://192.168.0.3/MillennialInvest/Site-v7/v1.4/public/index.php/Exchange/Order_Event_Manager/${url + id}`, { withCredentials: true }
		);
