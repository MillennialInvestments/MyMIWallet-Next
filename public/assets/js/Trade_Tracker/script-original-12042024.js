...
class TradeWindow {    
    constructor(holder) {
        console.log("Holder element:", holder);

        if (!holder) {
            console.error("TradeWindow: 'holder' is null or undefined. Initialization aborted.");
            return;
        }
        if (typeof holder.agd === "function") {
            holder.agd("tradeWindow");
        } else {
            console.warn("TradeWindow: 'agd' method not found on holder. Skipping agd application.");
        }
        ...
        // Safely initialize userPrefs
        const userPrefs = window.userPrefs || { sortings: [], selectedSorting: 0 };
        ...
        console.log("TradeWindow initialized successfully.");
    }
    /**
        ...
        this.d_saveChanges = async () => {
            try {
                ...
                Object.values(userPrefs.customFields).forEach((customField) => {
                    ...
                    delete dbObject[customField.name]; // Remove custom fields from dbObject
                });
        
                dbObject[gin("juf")] = jsUF && Object.keys(jsUF).length > 0
                    ? JSON.stringify(jsUF)
                    : "{}";
                ...
                if (!tradeWindow) {
                    throw new Error("Trade window is not defined for this row");
                }
                if (!this.state.table) {
                    throw new Error("Table is not defined for this row");
                }
                if (isParent && isChild) {
                    throw new Error("Trade is both parent and child");
                }
        
                // Save the parent first if it's a child trade
                if (isChild) {
                    const parentId = this.current[gin("29")];
                    if (!tradeWindow.allRowsObj.hasOwnProperty(parentId)) {
                        throw new Error(`The trade window doesn't contain the parent (${parentId}) of this trade`);
                    }
        
                    const parent = tradeWindow.allRowsObj[parentId];
                    const parentSaved = await parent.d_saveChanges();
                    if (!parentSaved) {
                        throw new Error("Failed to save the parent of this child trade");
                    }
                }
                ...
                if (data.status === "0") {
                    newAlert({ status: "error", message: "Saving the trade was unsuccessful" });
                    console.error("API: Error processing this data", data);
                    return false;
                }
        
                // Update local trade data based on response
                const updatedTrade = JSON.parse(data.message);
                Object.keys(updatedTrade).forEach((key) => {
                    if (this.current.hasOwnProperty(key)) {
                        this.current[key] = updatedTrade[key];
                    }
                });
        
                delete updatedTrade[gin("30")]; // Remove unnecessary data from response
                this.current = Object.assign(this.current, updatedTrade);
                this.current[gin("00p")] = this.current[gin("00i")]; // Update pseudo ID
        
                // Handle parent-child relationships
                if (isParent) {
                    const childIdList = JSON.parse(this.current[gin("30")]);
                    childIdList.forEach((childId) => {
                        if (tradeWindow.allRowsObj.hasOwnProperty(childId)) {
                            tradeWindow.allRowsObj[childId].current[gin("29")] = this.current[gin("00p")];
                            tradeWindow.allRowsObj[childId].origin[gin("29")] = this.current[gin("00p")];
                        } else {
                            console.error(`Child ID ${childId} missing in tradeWindow`);
                        }
                    });
                }
        
                // Refresh the origin object to match the modified current object
                this.updateCurrent("", 1);
                this.cancelChanges();
                return true;
            } catch (error) {
                newAlert({ status: "error", message: "Saving the trade was unsuccessful" });
                console.error("Error in d_saveChanges:", error);
                return false;
            }
        };
